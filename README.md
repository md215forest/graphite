...
- Cmd + Shift + G でウィンドウをカーソル位置に呼び戻す
- 修飾キーのダブルタップでウィンドウをカーソル位置に表示（既定: 右 Option 2連打、設定で変更可）
...
| Cmd + Shift + G | ウィンドウをカーソル位置に呼び戻す |
| 修飾キーのダブルタップ | ウィンドウをカーソル位置に表示（既定: 右 Option、⋯ メニューの Show at Cursor で変更） |

ダブルタップによる呼び戻しは、Graphite が前面にない間も動かすためにグローバルなキー監視を使います。そのため macOS の「入力監視（Input Monitoring）」許可が必要です。許可しない場合でも、Cmd + Shift + G による呼び戻しは権限不要で動作します。
...
App Store で配布する代わりに、リポジトリをクローンして自分でビルドし、`.app` として常用できます。`swift run` は開発用のアンバンドル実行（アクセサリプロセス扱い・識別子が不安定で入力監視の許可が維持されない）なので、永続利用には `.app` の作成を推奨します。

リポジトリを取得して、付属スクリプトでビルド・インストールします。

```sh
git clone <repository-url> graphite
cd graphite
./scripts/build-app.sh --install
```

これで `Graphite.app` を組み立て、アドホック署名し、`/Applications` にコピーします。`--install` を省くと `dist/Graphite.app` を作成するだけです。以後は Launchpad / Spotlight / Finder から起動できます。

初回のみ、次の設定が必要です。

- 初回起動時に Gatekeeper の警告が出たら、Finder で `Graphite.app` を右クリック →「開く」で一度許可します。
- 修飾キーのダブルタップで呼び戻す機能を使う場合は、システム設定 → プライバシーとセキュリティ → 入力監視 で Graphite を有効にします。

ログイン時に自動起動したい場合は、システム設定 → 一般 → ログイン項目 に `/Applications/Graphite.app` を追加します。

アップデートは、再度取得してビルドし直すだけです。
...
なお、アドホック署名では再ビルドのたびに署名識別子が変わるため、更新後に入力監視の許可を取り直す必要がある場合があります。再許可を不要にしたい場合は、自己署名証明書を作成して `scripts/build-app.sh` の `codesign --sign -` を証明書名に置き換えてください。
...
- Cmd + Shift + G brings the window back at the cursor
- Double-tap a modifier key to show the window at the cursor (default: Right Option, configurable)
...
| Cmd + Shift + G | Bring the window back at the cursor |
| Double-tap a modifier key | Show the window at the cursor (default: Right Option; change via Show at Cursor in the ⋯ menu) |

The double-tap gesture works even while Graphite is not frontmost, so it relies on a global key monitor and therefore requires macOS Input Monitoring permission. Cmd + Shift + G works without any permission as a fallback.
...
## Installation (Local Build for Permanent Use)

Instead of distributing through the App Store, you can clone the repository, build it yourself, and use it as a `.app`. `swift run` is for development only — it launches an unbundled, accessory-style process whose identity is unstable, so the Input Monitoring grant does not persist. For permanent use, build the `.app`.

Clone the repository and use the bundled script to build and install:
...
This assembles `Graphite.app`, ad-hoc signs it, and copies it to `/Applications`. Omit `--install` to only produce `dist/Graphite.app`. You can then launch it from Launchpad, Spotlight, or Finder.

First-time setup:

- If Gatekeeper warns on first launch, right-click `Graphite.app` in Finder and choose "Open" once to allow it.
- To use the double-tap show-at-cursor gesture, enable Graphite under System Settings → Privacy & Security → Input Monitoring.

To launch at login, add `/Applications/Graphite.app` under System Settings → General → Login Items.

To update, just pull and rebuild:
...
Because ad-hoc signing changes the code signature identity on every rebuild, you may need to re-grant Input Monitoring after an update. To avoid re-granting, create a self-signed certificate and replace `codesign --sign -` in `scripts/build-app.sh` with your certificate name.
